...
typ = 3;                    %rodzaj filtra 1-LP,2-HP,3-BP,4BS
...
%============ wyznaczenie parametrow wybranego filtra =====================

wp=1;
N=ceil(log10((10^(astop/10)-1)./(10^(apass/10)-1))/(2*log(ws/wp)));
                            %liczba wymaganych biegunow (str. 148 wz. 6.53)
w0=ws/(10^(astop/10)-1)^(1/(2*N));
                            %promien okregu w3dB, pulsacja filtra
                            %(str. 148 wz. 6.54)



%========= wyznaczenie biegunow transmitancji prototypowego filtra LP =====
dfi0=(2*pi)/(2*N);          %kat 
fi=pi/2+dfi0/2+(0:N-1)*dfi0;%katy biegunow
p=w0*exp(1j*fi);            %bieguny
z=[];                       %zera
wzm=real(prod(-p));         %wzmocnienie
a=poly(p);                  %bieguny (wsp. mianownika wielomianu) A(z)
