...
%=== wyznaczenie biegunow dolnoprzepustowego filtra prototypowego =========
wp=1;
Nreal=acosh(sqrt((10^(astop/10)-1)/(10^(apass/10)-1)))/acosh(ws/wp);
N=ceil(Nreal);              %liczba wymaganych biegunow (str. 159 wz. 6.63)

epsi=sqrt(1/(10^(astop/10)-1));
                            %pasmo zaporowe od w/w0=1 do nieskonczonosci
                            %str 161 wz 6.71
D=asinh(1/epsi)/N;
...
dfi0=(2*pi)/(2*N);          %kat
fi=pi/2+dfi0/2+(0:N-1)*dfi0;%katy biegunów
...
z=1./z;
p=1./p;                     %roznica pomiedzy f. czybyszeba typu I oraz II

b = wzm*poly(z)             %wielomian licznika B(z)
a = poly(p)                 %bieguny (wsp wielomianu mianownika A(z))
printsys(b,a,'s')           %wypisanie transmitancji w konsoli

%======= Porownanie funkcji z funkcjami matlaba ===========================
[NN,ww0]=cheb2ord(vp,vs,apass,astop,'s' );
blad_N=N-NN

%========== wykres zer oraz biegunow prototypu ============================

figure(1);
plot(real(p),imag(p),'xb',real(z),imag(z),'or');
grid;
title('Wykres zer i biegunow prototypu');
xlabel('Rzeczywiste');
ylabel('Urojone');
legend('bieguny','zera');
